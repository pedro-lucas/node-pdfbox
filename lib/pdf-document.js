...
const Field = require('./pdf-field');
const AcroForm = require('./pdf-acro-form');
...
class PDFDocument extends JavaObject {
  ...
  getForm() {
    return when.promise(function(resolve, reject) {
      this.javaObject.getDocumentSync().getDocumentCatalogSync().getAcroFormPromise()
      .then(function(form) {
        resolve(new AcroForm(form));
      })
      .catch(reject);
    }.bind(this));
  }
  ...
  getFormSync() {
    let form = this.javaObject.getDocumentSync().getDocumentCatalogSync().getAcroFormSync();
    return new AcroForm(form);
  }

  close() {
    let promise = this.javaObject.getDocumentSync().closePromise();
